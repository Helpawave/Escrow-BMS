...
export const UplineInput = ({ value, onChange, onEntriesChange, resetTrigger }: UplineInputProps) => {
  const { user } = useAuth();

  const [entries, setEntries] = useState<UplineEntry[]>([
  ...
  useEffect(() => {
    const loadSavedNames = async () => {
      ...
      try {
        const { data, error } = await db.savedNames.getSavedNames(user.id, 'uplines');
        ...
        console.error('Error loading saved upline names:', error);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    const saveToDatabase = async () => {
      ...
      try {
        ...
        if (hasNames) {
          ...
          const { error } = await db.savedNames.saveNames(user.id, 'uplines', namesToSave);
        ...
        console.error('Error saving upline names:', error);
      }
    };
  ...
  useEffect(() => {
    ...
    if (value === "" && directValue !== "" && !isLoading && hasInitiallyLoaded) {
      ...
      if (!hasExistingNames) {
        setEntries([{ id: '1', name: '', amount: '', isEditingName: true }]);
        setDirectValue("");
        setIsExpanded(false);
      } else {
  ...
  useEffect(() => {
    if (isExpanded) {
      // When expanded, always use entries total
      const entriesTotal = entries.reduce((sum, entry) => {
        const amount = parseFloat(entry.amount) || 0;
        return sum + amount;
      }, 0);
      const totalString = entriesTotal.toString();
      onChange(totalString);

      // Send entries to parent for saving
      const validEntries = entries
        .filter(entry => entry.name && entry.amount)
        .map(entry => ({ name: entry.name, value: parseFloat(entry.amount) || 0 }));
      onEntriesChange?.(validEntries);
    } else {
      // When collapsed, use direct value
  ...
  const updateEntry = (id: string, field: keyof UplineEntry, value: string | boolean) => {
  ...
  return (
    <div className="space-y-1">
      {/* Upline Display with Plus Button */}
      <div>
