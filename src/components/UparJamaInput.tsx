...
export const UparJamaInput = ({ value, onChange, onEntriesChange, resetTrigger }: UparJamaInputProps) => {
  const { user } = useAuth();

  const [entries, setEntries] = useState<UparJamaEntry[]>([
  ...
  useEffect(() => {
    const loadSavedNames = async () => {
      ...
      try {
        const { data, error } = await db.savedNames.getSavedNames(user.id, 'rtgs');
        ...
        console.error('Error loading saved RTGS names:', error);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    const saveToDatabase = async () => {
      ...
      try {
        ...
        if (hasNames) {
          ...
          const { error } = await db.savedNames.saveNames(user.id, 'rtgs', namesToSave);
        ...
        console.error('Error saving RTGS names:', error);
      }
    };
  ...
  useEffect(() => {
    ...
    if (value === "" && directValue !== "" && hasInitiallyLoaded) {
      // console.log('UparJamaInput: Resetting component after form clear');
  ...
  const updateEntry = (id: string, field: keyof UparJamaEntry, value: string | boolean) => {
  ...
  return (
    <div className="space-y-1">
      {/* Upar Jama Display with Plus Button */}
      <div>
