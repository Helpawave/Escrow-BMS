...
interface BankEntry {
  id: string;
  name: string;
  balance: string; // Changed from amount to balance
...
export const BankBalanceInput = ({ value, onChange, onEntriesChange, resetTrigger }: BankBalanceInputProps) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const loadSavedNames = async () => {
      ...
      try {
        const { data, error } = await db.savedNames.getSavedNames(user.id, 'banks');
        ...
        if (data && data.length > 0) {
          ...
          const uniqueNames = [...new Set(data.map((entry: any) => entry.name).filter((name: string) => name && name.trim() !== ''))] as string[];
          ...
          const restoredEntries = data.map((entry: any, index: number) => ({
            ...
            name: entry.name || '',
            balance: entry.balance || entry.amount || '0', // Handle both old and new keys, default to 0
        ...
        console.error('Error loading saved bank names:', error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    const saveToDatabase = async () => {
      ...
      try {
        ...
        if (hasNames) {
          ...
          const namesToSave = entries
            ...
            .map(entry => ({
              id: entry.id,
              name: entry.name,
              balance: entry.balance, // Changed from amount to balance
              isEditingName: entry.isEditingName
            }));

          const { error } = await db.savedNames.saveNames(user.id, 'banks', namesToSave);
        ...
        console.error('Error saving bank names:', error);
      }
    };
  ...
  useEffect(() => {
    ...
    if (!hasInitiallyLoaded && !isLoading) {
      setHasInitiallyLoaded(true); // Ensure it's set if not already
    }
  ...
  const removeEntry = (id: string) => {
    if (entries.length > 1) {
      ...
      setDirectValue(''); // Clear direct value too
    }
  };

  const updateEntry = (id: string, field: keyof BankEntry, value: string | boolean) => {
