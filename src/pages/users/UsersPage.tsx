...
export default function UsersPage() {
  ...
  const handleBulkDeleteUsers = async () => {
    ...
    if (!window.confirm(`Are you sure you want to remove all ${selectedUserIds.length} selected users?`)) return;
    ...
    try {
      ...
      try {
        ...
        if (saved) {
          let list = JSON.parse(saved);
          list = list.filter((item: any) => !selectedUserIds.includes(item.id));
          localStorage.setItem('synced_universal_users_v1', JSON.stringify(list));
        }
      } catch (lsErr) {}

      toast.success(`Removed ${selectedUserIds.length} selected users from directory`);
      setSelectedUserIds([]);
      await fetchUsers();
    } catch (err: any) {
      toast.error('Failed to perform bulk deletion', { description: err.message });
    } finally {
            ...
            <div className="flex items-center gap-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-4 py-2 rounded-xl">
              ...
              {selectedUserIds.length > 0 && (
                <button
                  ...
                  className="px-3 py-1 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-lg shadow-xs transition-all flex items-center gap-1.5 cursor-pointer"
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredUsers.map((user) => (
              <div 
                key={user.id} 
                className={cn(
                  "bg-white dark:bg-slate-900 border rounded-2xl p-5 shadow-xs space-y-4 transition-all flex flex-col justify-between relative",
