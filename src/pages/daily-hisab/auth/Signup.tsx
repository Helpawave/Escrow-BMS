...
import { Eye, EyeOff, Mail, User, Phone, DollarSign, Loader2 } from 'lucide-react';
...
export const Signup = () => {
  ...
  useEffect(() => {
    if (user && user.is_allowed) {
      ...
      console.log('[SIGNUP_PAGE]: User already authenticated and approved, redirecting to dashboard')
      navigate('/dashboard')
    }
  }, [user, navigate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    console.log('[SIGNUP_FORM]: Form submitted with email:', formData.email, 'name:', formData.name);
    
    if (formData.password !== formData.confirmPassword) {
      console.log('[SIGNUP_FORM]: Password mismatch error');
      toast({
    ...
    if (result.success) {
      ...
      toast({
        ...
        description: "Your account is pending admin approval. You'll be redirected to the approval page.",
      });
      // Don't navigate to dashboard - let UserLayout handle the approval check
    } else {
  ...
  const handleGoogleSignup = async () => {
    try {
      // Import the Google auth function from repository
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted flex items-center justify-center p-3 sm:p-6">
      <div className="w-full max-w-md animate-fade-in">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">Join FinTrack and start tracking your finances</p>
        </div>
        ...
        <Card className="shadow-card">
          <CardHeader className="space-y-1">
            ...
            <CardDescription>
              Create your account to get started with financial tracking
            </CardDescription>
          </CardHeader>
